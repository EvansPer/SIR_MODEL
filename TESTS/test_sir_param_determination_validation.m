...
% with A_true and B_true. If the assert condition is false, then the test fails.
...
% sir_param_determination may find.
    ...
    assert(abs(A_true - A) < 1e-8 && abs(B_true - B) < 1e-8, "Test failed: function did not recover correct parameters");
